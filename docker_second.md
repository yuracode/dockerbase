...
| `docker container run -p 8080:80 nginx` | ブラウザで `localhost:8080` を開き Nginx を確認 |
| `docker container ls` / `docker container ls -a` | 起動中・停止済みコンテナの一覧を確認 |
| `docker container stop [ID]` / `docker container rm [ID]` | コンテナを停止・削除 |

> **キーワード：** イメージとコンテナの違い、`docker container run` = pull + create + start
...
> **ポイント：** `docker container run` は Create と Start を同時に行います。
...
`docker image build` に失敗した場合、エラーメッセージを確認してどのステップで止まったかを特定します。
...
`docker container run` してもすぐ終了してしまう場合はログを確認します。

```bash
# ログを確認する
docker container logs <コンテナ名 or ID>

# リアルタイムで追跡する
docker container logs -f <コンテナ名 or ID>
```
...
| `docker image build` でオリジナルイメージを作成 | イメージとは「自分専用のサーバー設計書」 |
| `docker container logs` でエラーを確認 | コンテナ内の状態をログで把握する方法 |
